...
# ================= SAFE CONFIG LOADER =================
def get_config(env_key: str, filename: str = "") -> str:
...
CLIENT_ID = get_config("CLIENT_ID", "client_id.txt")
ACCESS_TOKEN = get_config("ACCESS_TOKEN", "access_token.txt")
TELEGRAM_TOKEN = get_config("TELEGRAM_TOKEN")
CHAT_ID = get_config("CHAT_ID")

if not CLIENT_ID or not ACCESS_TOKEN:
    raise Exception("Missing CLIENT_ID or ACCESS_TOKEN. Set them in Railway Variables.")
...
def check_auth():
    try:
        ...
        if profile.get("s") != "ok":
            raise Exception(str(profile))
        return profile
    except Exception as e:
        raise Exception(f"FYERS auth failed: {e}")
...
def main():
    ...
    msg.extend(build_oi_lines(parsed_rows, 12))
